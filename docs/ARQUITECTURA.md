...
- Las **reglas de Firestore** se publican con la CLI de Firebase (`npx firebase-tools deploy --only firestore:rules`), configurada en `firebase.json` y `.firebaserc`. Antes se copiaban a mano en la consola en cada spec, que era el paso más fácil de olvidar: si el código va por delante de las reglas, la app falla en producción con errores de permisos difíciles de leer. Requiere haber hecho `firebase login` una vez en el equipo.
