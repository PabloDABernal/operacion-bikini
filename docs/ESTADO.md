...
- **Fix del 21 de agosto, solución definitiva: la barra de navegación desaparecía/saltaba en Ejercicio, en móvil.** Detectado probando la spec 038. Costó varios intentos: los tres primeros (traducir la barra según `visualViewport` solo en eventos `resize`/`scroll`, retrasar el `scrollTo(0,0)` a `requestAnimationFrame`, y luego enganchar bien esa misma cuenta de `visualViewport` dentro de `abrirPestana()`) eran parches sobre `position: fixed`, que es justo lo que causaba el problema: en algunos navegadores móviles, `fixed` se coloca contra un viewport "de diseño" que no coincide con lo que de verdad se ve (confirmado con un panel de diagnóstico temporal: al entrar en Ejercicio, `window.innerHeight` saltaba de 681 a 733 mientras `visualViewport.height` se quedaba en 681, un hueco de 52 px justo donde vive la barra). Parcheado con JS quedaba **visualmente inconsistente**: unas veces la barra estaba pegada abajo del todo, otras corregida 52 px más arriba con un hueco visible debajo — el propio usuario lo notó como "cutre" en dos capturas comparadas. **La solución de verdad**: `#nav-inferior` pasó de `position: fixed` a `position: sticky` (`styles.css`), que la deja en el flujo normal del documento en vez de anclarla contra un viewport que puede no coincidir con lo real. Con esto el JS de `visualViewport`/`ajustarBarraInferior()` sobraba entero y se quitó — cero líneas de compensación, la barra ya no puede desincronizarse porque no depende de ningún cálculo de viewport. Para que siga yendo de borde a borde pese al padding lateral del `body`, lleva un margen negativo (`margin-left/right: calc(-1 * var(--e4))`), anulado en escritorio donde vive junto a la cabecera. El padding inferior del `body` que reservaba hueco para la barra fija también se redujo, porque con sticky la barra ya ocupa su propio sitio en el flujo, no se superpone. Verificado con Playwright (`file://index.html`, sin red): en móvil la barra queda de borde a borde y se mantiene pegada abajo incluso a mitad de scroll de una página muy larga; en escritorio, la rejilla de la cabecera no cambia.

**Segunda vuelta, mismo día**: con sticky ya sin desincronizarse, apareció el efecto contrario — en secciones con poco contenido (Fotos, y Ejercicio con pocos registros) la barra se quedaba pegada justo debajo del contenido en vez de en el borde real de la pantalla, con hueco visible debajo, porque sticky respeta el flujo normal y no fuerza nada si el contenido no llega a ocupar toda la pantalla. Arreglado dándole a `#pantalla-principal` `display: flex; flex-direction: column; min-height: 100dvh` y a `.seccion.activa` `flex: 1 0 auto`, para que la sección activa crezca y llene lo que sobre — así la barra cae siempre en el borde real, tenga la sección mucho o poco contenido. Anulado en escritorio (`min-height: auto` dentro del `@media` de 48rem), donde la barra ya no vive abajo. Verificado con Playwright en las seis secciones con contenido vacío (el caso más corto posible): la barra queda pegada al borde real en las seis, sin excepción, y sigue sticky durante el scroll de una página larga. **Pendiente de que el usuario lo confirme en su móvil.**
