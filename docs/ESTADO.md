...
- **Parpadeo de la barra de navegación en móvil al cambiar de sección** (detectado probando la spec 038, 21 de agosto): en pantallas largas como Ejercicio, `abrirPestana()` hace `window.scrollTo(0, 0)` al cambiar de pestaña; si el navegador (Brave, en el caso probado) tenía su propia barra de direcciones escondida por scroll previo, al volver a 0 de golpe el navegador la reexpande y, durante esa animación, `#nav-inferior` (con `position: fixed; bottom: 0`) se ve un instante recolocándose con ella. Se confirma que **se autocorrige** en cuanto se toca o se hace scroll — es el navegador redibujando su propio chrome, no un fallo del CSS/JS de la app. No se ha tocado nada: no es fiable "arreglarlo" desde la web sin arriesgar a introducir un problema peor. Aceptado como límite conocido.
