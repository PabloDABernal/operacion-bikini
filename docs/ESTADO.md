...
- **Fix del 21 de agosto: la barra de navegación desaparecía en Ejercicio, en móvil.** Detectado probando la spec 038: en Comidas la barra se veía pegada abajo como siempre, pero en Ejercicio no aparecía en absoluto (dos capturas comparadas confirmaron que no era un parpadeo, sino que faltaba del todo). Causa: `#nav-inferior` usa `position: fixed; bottom: 0`, y algunos navegadores móviles la posicionan contra el *viewport de diseño* (el que habría sin la barra de direcciones del propio navegador) en vez de contra el *viewport visual* (lo que de verdad se ve en pantalla); cuando la barra del navegador está expandida, eso deja el `bottom: 0` de la app fuera de la parte visible. Ejercicio lo disparaba más que otras secciones por ser la de contenido más alto (un campo más que Comidas, "Minutos"), quedando justo al borde de necesitar scroll. Arreglado con `window.visualViewport`: en `js/app.js`, un listener de `resize`/`scroll` de `visualViewport` calcula cuánto hay que subir la barra (`translateY`) para que se quede pegada a lo que el navegador enseña de verdad, y no hace nada si `#nav-inferior` no es `position: fixed` (en escritorio ya no lo es, spec 009). Sin `window.visualViewport` (navegadores muy antiguos) se queda el comportamiento de siempre. **Pendiente de que el usuario lo confirme en su móvil.**
