...
- **Fix del 21 de agosto: la barra de navegación desaparecía en Ejercicio, en móvil.** Detectado probando la spec 038. Costó tres intentos: los dos primeros (traducir la barra según `visualViewport` solo en eventos `resize`/`scroll`, y retrasar el `scrollTo(0,0)` a `requestAnimationFrame`) no lo arreglaron porque eran suposiciones sin medir nada. Un panel de diagnóstico temporal pintado en pantalla (quitado ya) dio el dato real: al entrar en Ejercicio, `window.innerHeight` saltaba de 681 a 733 mientras `window.visualViewport.height` —lo que de verdad se ve— se quedaba en 681. Un hueco de 52 px, justo donde vive `#nav-inferior` (`position: fixed; bottom: 0`): el navegador la coloca contra el `innerHeight` "de diseño", no contra lo que realmente se ve, y queda fuera de la pantalla. Ejercicio lo disparaba más que otras secciones por ser la de contenido más alto (un campo más que Comidas, "Minutos"). **La cuenta del primer intento ya era correcta**, pero nunca se ejecutaba al cambiar de pestaña: solo escuchaba eventos que no saltan cuando lo que cambia es la altura del documento, no el tamaño de la ventana. Arreglado enganchando esa misma cuenta (`ajustarBarraInferior()`, en `js/app.js`) directamente en `abrirPestana()`, justo tras el `scrollTo(0,0)`. **Pendiente de que el usuario lo confirme en su móvil.** Si aparece el mismo síntoma en otra sección alguna vez, el panel de diagnóstico (código en el historial de git, commit `5829e91`) es la forma de medirlo con datos antes de tocar nada a ciegas.
