...
- **Parpadeo de la barra de navegación en móvil, específico de Ejercicio** (detectado probando la spec 038, 21 de agosto, confirmado con pruebas con Playwright el mismo día): al entrar en Ejercicio, `#nav-inferior` (con `position: fixed; bottom: 0`) se ve un instante recolocándose, y se autocorrige en cuanto se toca la pantalla o se hace scroll. Medido con Playwright cargando `index.html` y forzando cada sección (sin red, sin login): **la barra está correctamente fijada abajo en las seis secciones, en el mismo sitio siempre** — no hay ningún bug de CSS. Lo que sí es distinto: Ejercicio es la pantalla con el contenido más alto de las seis (su formulario tiene un campo más que el de Comidas, "Minutos"), y con la lista de ejercicios ya cargada esa altura queda justo al borde de lo que cabe sin hacer scroll en un móvil normal. Ese límite es el que hace que el navegador decida mostrar/ocultar su propia barra de direcciones al aterrizar ahí — es el navegador redibujando su propio chrome, no algo que se pueda arreglar de forma fiable desde el CSS de la app sin arriesgar a introducir un problema peor. Aceptado como límite conocido, específico de Ejercicio por su altura.
